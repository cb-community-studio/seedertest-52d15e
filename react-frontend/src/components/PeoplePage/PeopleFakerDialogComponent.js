...
import peopleFakerFactory from "./peopleFakerFactory";
...
const PeopleFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = peopleFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("people").create(elem));
...
export default connect(null, mapDispatch)(PeopleFakerDialogComponent);
